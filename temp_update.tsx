...
const GroupManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      console.log('Fetching module permissions mapping...');
      const response = await apiClient.get(endpoints.modulePermissions);
      console.log('Module mapping received:', response.data);
      ...
      if (response.data && Object.keys(response.data).length > 0) {
        ...
        console.log('Setting available modules:', modules);
        ...
        console.log('Setting default modules:', defaultModules);
  ...
  const openEditDialog = (group: Group) => {
    console.log(`Opening edit dialog for group: ${group.name}`, group);
    ...
    setAvailableModules(prev => {
      const updatedModules = prev.map(module => {
        ...
        const hasAccess = groupsWithAccess.includes(group.name);
        console.log(`Module ${module.key}: Current access = ${hasAccess}, Groups with access:`, groupsWithAccess);
        return {
          ...module,
          allowed: hasAccess
        };
      });
      console.log('Updated modules with access flags:', updatedModules);
    ...
    try {
      ...
      if (dialogMode === 'create') {
        // Create new group
        const createResponse = await apiClient.post(endpoints.groups, { name: groupName });
      ...
      try {
        // Create a modules object with true/false values
        const modulePermissions: { [key: string]: boolean } = {};
        availableModules.forEach(module => {
          modulePermissions[module.key] = module.allowed;
        });
        
        // Log the module permissions being sent
        console.log('Updating module permissions for group', groupName, modulePermissions);
        
        // Send the module permissions update
        await apiClient.post(endpoints.updateGroupModules, {
          group_name: groupName,
          modules: modulePermissions
        });
        
        // Refresh module mapping to update UI
        await fetchModuleMapping();
        
        enqueueSnackbar('Module permissions updated successfully', { variant: 'success' });
      } catch (permError: any) {
        console.error('Error updating module permissions:', permError);
  ...
  const handleModuleToggle = (moduleKey: string) => {
    console.log(`Toggling module permission for: ${moduleKey}`);
    setAvailableModules(prev => {
      const newModules = prev.map(module => {
        if (module.key === moduleKey) {
          const newValue = !module.allowed;
          console.log(`Setting ${moduleKey} access to: ${newValue}`);
          return { ...module, allowed: newValue };
        }
        return module;
      });
      console.log('Updated modules after toggle:', newModules);
      return newModules;
    });
  };
