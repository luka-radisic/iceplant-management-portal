...
activate_venv() {
    ...
    elif [ -f "$BASE_DIR/venv/Scripts/activate" ]; then
...
if [ ! -f "$BASE_DIR/requirements.installed" ]; then
    echo -e "${YELLOW}Installing Python dependencies...${NC}"
    "$BASE_DIR/venv/bin/pip" install -r "$BASE_DIR/requirements.txt"
    touch "$BASE_DIR/requirements.installed"
fi
...
"$BASE_DIR/venv/bin/python" "$BASE_DIR/manage.py" migrate

# Start Django server in the background
echo -e "${GREEN}Starting Django backend server...${NC}"
"$BASE_DIR/venv/bin/python" "$BASE_DIR/manage.py" runserver 8000 &
DJANGO_PID=$!
